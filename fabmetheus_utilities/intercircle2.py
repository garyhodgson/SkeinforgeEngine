...
def addEndCap(begin, end, points, radius):
	...
	totalAngle = 0.0
	angle = euclidean2.globalTau / float(numberOfSides)
	...
	for pointIndex in xrange(len(path) - 1):
		...
		if endIndex < len(path):
			...
			if centerBegin != None and centerEnd != None:
				...
				if euclidean2.getCrossProduct(centerBegin, centerEnd) < 0.0:
...
def addInsetPointFromClockwiseTriple(begin, center, end, loop, radius):
	...
	dotProduct = euclidean2.getDotProduct(centerMinusBeginClockwise, centerClockwise)
...
def addOrbits( distanceFeedRate, loop, orbitalFeedRatePerSecond, temperatureChangeTime, z ):
	...
	while timeInOrbit < temperatureChangeTime:
		...
		timeInOrbit += euclidean2.getLoopLength(loop) / orbitalFeedRatePerSecond
...
def directLoop(isWiddershins, loop):
	'Direct the loop.'
	if euclidean2.isWiddershins(loop) != isWiddershins:
		loop.reverse()
...
def getAroundsFromPathPoints(points, radius, thresholdRatio=0.9):
	...
	for center in centers:
		if euclidean2.isWiddershins(center):
			arounds.append(euclidean2.getSimplifiedPath(center, radius))
	return arounds
...
def getCircleIntersectionsFromCircleNodes(circleNodes):
	...
	for circleNode in circleNodes:
		euclidean2.addElementToPixelListFromPoint(circleNode, pixelTable, circleNode.dividedPoint)
	...
	for circleNodeIndex in xrange(len(circleNodes)):
		...
		if circleNodeIndexMinusOne >= 0:
			...
			euclidean2.addElementToPixelListFromPoint(circleNodeAdditional, accumulatedCircleNodeTable, 0.5 * circleNodeAdditional.dividedPoint)
...
def getCircleNodesFromPoints(points, radius):
	...
	points = euclidean2.getAwayPoints(points, radius)
	for point in points:
...
def getInsetLoopsFromLoop(loop, radius, thresholdRatio=0.9):
	...
	isInset = radius > 0
	insetLoops = []
	isLoopWiddershins = euclidean2.isWiddershins(loop)
	arounds = getAroundsFromLoop(loop, radius, thresholdRatio)
	for around in arounds:
		leftPoint = euclidean2.getLeftPoint(around)
		shouldBeWithin = (isInset == isLoopWiddershins)
		if euclidean2.isPointInsideLoop(loop, leftPoint) == shouldBeWithin:
			if isLoopWiddershins != euclidean2.isWiddershins(around):
				around.reverse()
...
def getInsetLoopsFromVector3Loop(loop, radius, thresholdRatio=0.9):
	...
	if len(loop) < 2:
		return [loop]
	loopComplex = euclidean2.getComplexPath(loop)
	loopComplexes = getInsetLoopsFromLoop(loopComplex, radius)
	return euclidean2.getVector3Paths(loopComplexes, loop[0].z)
...
def getInsetSeparateLoopsFromLoops(inset, loops, thresholdRatio=0.9):
	...
	for around in arounds:
		leftPoint = euclidean2.getLeftPoint(around)
		if isInset == euclidean2.getIsInFilledRegion(loops, leftPoint):
			if isInset:
				around.reverse()
...
def getLargestCenterOutsetLoopFromLoop(loop, radius, thresholdRatio=0.9):
	...
	for center in centers:
		...
		if isLargeSameDirection(outset, center, radius):
			if euclidean2.isPathInsideLoop(loop, outset) != euclidean2.isWiddershins(loop):
				centerOutset = CenterOutset(center, outset)
				outsetArea = abs(euclidean2.getAreaLoop(outset))
				if outsetArea > largestOutsetArea:
					largestOutsetArea = outsetArea
					largestCenterOutset = centerOutset
	if largestCenterOutset == None:
		return None
	largestCenterOutset.center = euclidean2.getSimplifiedLoop(largestCenterOutset.center, radius)
...
def getLargestInsetLoopFromLoop(loop, radius):
	...
	return euclidean2.getLargestLoop(loops)
...
def getSimplifiedInsetFromClockwiseLoop(loop, radius):
	...
	return getWithoutIntersections(euclidean2.getSimplifiedLoop(inset, radius))
...
def isLargeSameDirection( inset, loop, radius ):
	...
	if euclidean2.isWiddershins(inset) != euclidean2.isWiddershins(loop):
		return False
	return euclidean2.getMaximumSpan(inset) > 2.01 * abs(radius)
...
def isLoopIntersectingLoop( anotherLoop, loop ):
	...
	for pointIndex in xrange(len(loop)):
		...
		normalizedSegment = euclidean2.getNormalized(segment)
		segmentYMirror = complex(normalizedSegment.real, -normalizedSegment.imag)
		segmentFirstPoint = segmentYMirror * pointFirst
		segmentSecondPoint = segmentYMirror * pointSecond
		if euclidean2.isLoopIntersectingInsideXSegment( anotherLoop, segmentFirstPoint.real, segmentSecondPoint.real, segmentYMirror, segmentFirstPoint.imag ):
			return True
	return False
...
def removeIntersection( loop ):
	...
	for pointIndex, ahead in enumerate(loop):
		...
		if normalizedSegmentLength > 0.0:
			...
			xIntersection = euclidean2.getXIntersectionIfExists( aheadRotated, aheadMidpointRotated, y )
...
class BoundingLoop:
	...
	def getFromLoop( self, loop ):
		...
		self.loop = loop
		self.maximum = euclidean2.getMaximumByComplexPath(loop)
		self.minimum = euclidean2.getMinimumByComplexPath(loop)
		return self
	...
	def isEntirelyInsideAnother( self, anotherBoundingLoop ):
		...
		for point in self.loop:
			if euclidean2.getNumberOfIntersectionsToLeft( anotherBoundingLoop.loop, point ) % 2 == 0:
				return False
	...
	def isOverlappingAnother( self, anotherBoundingLoop ):
		...
		if self.isRectangleMissingAnother( anotherBoundingLoop ):
			return False
		for point in self.loop:
			if euclidean2.getNumberOfIntersectionsToLeft( anotherBoundingLoop.loop, point ) % 2 == 1:
				return True
		for point in anotherBoundingLoop.loop:
			if euclidean2.getNumberOfIntersectionsToLeft( self.loop, point ) % 2 == 1:
				return True
...
class CircleIntersection:
	...
	def getCircleIntersectionAhead(self):
		...
		for circleIntersection in circleIntersections:
			if not circleIntersection.steppedOn:
				circleIntersectionRelativeToMidpoint = euclidean2.getNormalized(circleIntersection.positionRelativeToBehind + self.aheadMinusBehind)
				dot = euclidean2.getDotProduct(self.demichord, circleIntersectionRelativeToMidpoint)
				if dot > largestDot:
					largestDot = dot
	...
	def isWithinCircles(self, pixelTable):
		...
		squareValues = euclidean2.getSquareValuesFromPoint(pixelTable, absolutePosition)
...
#		return '%s, %s, %s' % (self.index, self.dividedPoint, len(self.circleIntersections)) # when debugging bring back index
	...
	def getWithinNodes(self, pixelTable):
		...
		withinNodes = []
		squareValues = euclidean2.getSquareValuesFromPoint(pixelTable, 0.5 * self.dividedPoint)
		for squareValue in squareValues:
