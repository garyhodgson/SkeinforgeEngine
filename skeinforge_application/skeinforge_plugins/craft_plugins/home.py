...
class HomeSkein:
	...
	def addFloat( self, begin, end ):
		...
		self.gcode.addLine( self.gcode.getLinearGcodeMovementWithFeedRate( self.travelFeedRateMinute, closeToEnd.dropAxis(), closeToEnd.z ) )

	def addHopUp(self, location):
		"Add hop to highest point."
		locationUp = Vector3( location.x, location.y, self.highestZ )
		self.gcode.addLine( self.gcode.getLinearGcodeMovementWithFeedRate( self.travelFeedRateMinute, locationUp.dropAxis(), locationUp.z ) )
	...
	def addHomeTravel( self, splitLine ):
		...
		if self.extruderActive:
			self.gcode.addLine('M103')
		self.addHopUp( self.oldLocation )
		self.gcode.addLinesSetAbsoluteDistanceMode( self.homingLines )
		self.addHopUp( self.oldLocation )
		self.addFloat( self.oldLocation, location )
		if self.extruderActive:
			self.gcode.addLine('M101')
	...
	def parseInitialization( self, repository ):
		...
		for self.lineIndex in xrange(len(self.lines)):
			...
			if firstWord == '(</extruderInitialization>)':
				self.gcode.addLine('(<procedureName> home </procedureName>)')
				return
