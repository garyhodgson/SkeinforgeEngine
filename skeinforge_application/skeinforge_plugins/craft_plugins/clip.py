...
class ClipSkein:
	...
	def addGcodeFromThreadZ( self, thread, z ):
		...
		if len(thread) > 0 :
			self.gcode.addGcodeMovementZWithFeedRate( self.travelFeedRateMinute, thread[0], z )
		else:
		...
		for point in thread[1 :]:
			self.gcode.addGcodeMovementZWithFeedRate( self.feedRateMinute, point, z )
...
#			self.addGcodeFromThreadZ( self.loopPath.path, self.loopPath.z )
			for point in self.loopPath.path:
				self.gcode.addGcodeMovementZWithFeedRate( self.feedRateMinute, point, self.loopPath.z )
	...
	def parseInitialization( self, clipRepository ):
		...
		for self.lineIndex in xrange(len(self.lines)):
			...
			if firstWord == '(</extruderInitialization>)':
				self.gcode.addLine('(<procedureName> clip </procedureName>)')
				return
