...
from fabmetheus_utilities import archive, euclidean2, gcodec, intercircle2
...
def getCraftedText(fileName, text, gcode):
	...
	FillSkein2(gcode).getCraftedGcode()
	return text 
...
def addAroundGridPoint(arounds, gridPoint, gridPointInsetX, gridPointInsetY, gridPoints, gridSearchRadius, isBothOrNone, isDoubleJunction, isJunctionWide, paths, pixelTable, width):
	...
	for aroundIndex in xrange(len(arounds)):
		...
		for pointIndex in xrange(len(loop)):
			...
			yIntersection = euclidean22.getYIntersectionIfExists(pointFirst, pointSecond, gridPoint.real)
	...
	yIntersectionPaths = []
	gridPixel = euclidean22.getStepKeyFromPoint(gridPoint / width)
	segmentFirstPixel = euclidean22.getStepKeyFromPoint(complex(gridPoint.real, segmentFirstY) / width)
	segmentSecondPixel = euclidean2.getStepKeyFromPoint(complex(gridPoint.real, segmentSecondY) / width)
	pathIndexTable = {}
...
def addLoop(infillWidth, infillPaths, loop, rotationPlaneAngle):
	...
	simplifiedLoop = euclidean2.getSimplifiedLoop(loop, infillWidth)
	if len(simplifiedLoop) < 2:
		return
	simplifiedLoop.append(simplifiedLoop[0])
	planeRotated = euclidean2.getPointsRoundZAxis(rotationPlaneAngle, simplifiedLoop)
	infillPaths.append(planeRotated)

def addPath(infillWidth, infillPaths, path, rotationPlaneAngle):
	'Add simplified path to fill.'
	simplifiedPath = euclidean2.getSimplifiedPath(path, infillWidth)
	if len(simplifiedPath) < 2:
		return
	planeRotated = euclidean2.getPointsRoundZAxis(rotationPlaneAngle, simplifiedPath)
	infillPaths.append(planeRotated)
...
def addPointOnPath(path, pathIndex, pixelTable, point, pointIndex, width):
	...
	if pointIndex < len(path) and pointIndexMinusOne >= 0:
		...
		euclidean2.addValueSegmentToPixelTable(begin, end, segmentTable, pathIndex, width)
		euclidean2.removePixelTableFromPixelTable(segmentTable, pixelTable)
	if pointIndexMinusOne >= 0:
		begin = path[ pointIndexMinusOne ]
		euclidean2.addValueSegmentToPixelTable(begin, point, pixelTable, pathIndex, width)
	if pointIndex < len(path):
		end = path[pointIndex]
		euclidean2.addValueSegmentToPixelTable(point, end, pixelTable, pathIndex, width)
...
def createExtraFillLoops(nestedRing, radius, shouldExtraLoopsBeAdded):
	...
	#print "allFillLoops",allFillLoops
...
def getExtraFillLoops(loops, radius):
	...
	extraFillLoops = []
	centers = intercircle2.getCentersFromPoints(intercircle2.getPointsFromLoops(loops, greaterThanRadius), greaterThanRadius)
	for center in centers:
		inset = intercircle2.getSimplifiedInsetFromClockwiseLoop(center, radius)
		if intercircle2.isLargeSameDirection(inset, center, radius):
			if euclidean2.getIsInFilledRegion(loops, euclidean2.getLeftPoint(inset)):
				inset.reverse()
...
def getYIntersectionInsideYSegment(segmentFirstY, segmentSecondY, beginComplex, endComplex, x):
	...
	yIntersection = euclidean2.getYIntersectionIfExists(beginComplex, endComplex, x)
	if yIntersection == None:
		return None
...
def isAddedPointOnPathFree(path, pixelTable, point, pointIndex, width):
	...
	if pointIndexMinusOne >= 0:
		...
		if pointIndex < len(path):
			...
			euclidean2.addValueSegmentToPixelTable(begin, end, maskTable, None, width)
		segmentTable = {}
		euclidean2.addSegmentToPixelTable(point, begin, segmentTable, 0.0, 2.0, width)
		if euclidean2.isPixelTableIntersecting(pixelTable, segmentTable, maskTable):
			return False
...
def isAddedPointOnPathIntersectingPath(begin, path, point, pointIndex):
	...
	if euclidean2.isXSegmentIntersectingPath(path[ max(0, pointIndex - 20) : pointIndex ], pointRotated.real, beginRotated.real, segmentYMirror, pointRotated.imag):
		return True
	return euclidean2.isXSegmentIntersectingPath(path[ pointIndex + 1 : pointIndex + 21 ], pointRotated.real, beginRotated.real, segmentYMirror, pointRotated.imag)
...
def isIntersectingLoopsPaths(loops, paths, pointBegin, pointEnd):
	...
	segmentYMirror = complex(normalizedSegment.real, -normalizedSegment.imag)
	pointBeginRotated = euclidean2.getRoundZAxisByPlaneAngle(segmentYMirror, pointBegin)
	pointEndRotated = euclidean2.getRoundZAxisByPlaneAngle(segmentYMirror, pointEnd)
	if euclidean2.isLoopListIntersectingInsideXSegment(loops, pointBeginRotated.real, pointEndRotated.real, segmentYMirror, pointBeginRotated.imag):
		return True
	return euclidean2.isXSegmentIntersectingPaths(paths, pointBeginRotated.real, pointEndRotated.real, segmentYMirror, pointBeginRotated.imag)
...
def isSegmentCompletelyInAnIntersection(segment, xIntersections):
	...
	for xIntersectionIndex in xrange(0, len(xIntersections), 2):
		...
		if euclidean2.isSegmentCompletelyInX(segment, surroundingXFirst, surroundingXSecond):
			return True
	return False
...
def isSharpCorner(beginComplex, centerComplex, endComplex):
	...
	return euclidean2.getDotProduct(centerBeginComplex, centerEndComplex) > 0.9
...
def isSidePointAdded(pixelTable, closestPath, closestPathIndex, closestPointIndex, layerExtrusionWidth, removedEndpointPoint, width):
	...
	euclidean2.addValueSegmentToPixelTable(pointBegin, pointEnd, maskTable, None, width)
	euclidean2.addValueSegmentToPixelTable(closest, removedEndpointPoint, closestSegmentTable, None, width)
	euclidean2.addValueSegmentToPixelTable(sidePoint, farthest, toPerpendicularTable, None, width)
	if euclidean2.isPixelTableIntersecting(pixelTable, toPerpendicularTable, maskTable) or euclidean2.isPixelTableIntersecting(closestSegmentTable, toPerpendicularTable, maskTable):
		sidePoint = removedEndpointPoint - perpendicular
		toPerpendicularTable = {}
		euclidean2.addValueSegmentToPixelTable(sidePoint, farthest, toPerpendicularTable, None, width)
		if euclidean2.isPixelTableIntersecting(pixelTable, toPerpendicularTable, maskTable) or euclidean2.isPixelTableIntersecting(closestSegmentTable, toPerpendicularTable, maskTable):
...
class FillSkein2:
	...
	def getCraftedGcode(self):
		...
		#self.gcodeCodec.addLines(self.lines[ self.shutdownLineIndex : ])
	...
	def addFill(self, layerIndex, rotatedLayer):
		...
		layerRotation = self.getLayerRotation2(layerIndex, rotatedLayer)
		...
		fillLoops = euclidean2.getFillOfSurroundings(nestedRings, None)
		...
		for loop in fillLoops:
			...
			planeRotatedPerimeter = euclidean2.getPointsRoundZAxis(reverseRotation, loop)
			rotatedLoops.append(planeRotatedPerimeter)
			centers = intercircle2.getCentersFromLoop(planeRotatedPerimeter, slightlyGreaterThanFill)
			euclidean2.addLoopToPixelTable(planeRotatedPerimeter, pixelTable, aroundWidth)
			for center in centers:
				alreadyFilledInset = intercircle2.getSimplifiedInsetFromClockwiseLoop(center, layerFillInset)
				if intercircle2.isLargeSameDirection(alreadyFilledInset, center, layerFillInset):
					alreadyFilledLoop.append(alreadyFilledInset)
					around = intercircle2.getSimplifiedInsetFromClockwiseLoop(center, aroundInset)
					if euclidean2.isPathInsideLoop(planeRotatedPerimeter, around) == euclidean2.isWiddershins(planeRotatedPerimeter):
						around.reverse()
						arounds.append(around)
						euclidean2.addLoopToPixelTable(around, pixelTable, aroundWidth)
		 
		if len(arounds) < 1:
			self.addThreadsBridgeLayer(layerIndex, nestedRings, rotatedLayer)
			return
		 
		back = euclidean2.getBackOfLoops(arounds)
		front = euclidean2.getFrontOfLoops(arounds)
		...
		self.horizontalSegmentLists = euclidean2.getHorizontalSegmentListsFromLoopLists(alreadyFilledArounds, front, numberOfLines, rotatedLoops, self.layerExtrusionWidth)
		...
		if len(surroundingCarves) >= self.doubleSolidSurfaceThickness:
			...
			self.frontOverWidth = euclidean2.getFrontOverWidthAddXListYList(front, surroundingCarves, numberOfLines, xIntersectionIndexLists, self.layerExtrusionWidth, self.yList)
			for fillLine in xrange(len(self.horizontalSegmentLists)):
				xIntersectionIndexList = xIntersectionIndexLists[fillLine]
				surroundingXIntersections = euclidean2.getIntersectionOfXIntersectionIndexes(self.doubleSolidSurfaceThickness, xIntersectionIndexList)
		...
		paths = euclidean2.getPathsFromEndpoints(endpoints, 5.0 * self.layerExtrusionWidth, pixelTable, aroundWidth)
		#print "paths",paths
		...
		paths = euclidean2.getConnectedPaths(paths, pixelTable, aroundWidth)
	...
	def addRotatedCarve(self, currentLayer, layerDelta, reverseRotation, surroundingCarves):
		...
		rotatedCarve = []
		for nestedRing in nestedRings:
			planeRotatedLoop = euclidean2.getPointsRoundZAxis(reverseRotation, nestedRing.getXYBoundaries())
		...
		rotatedCarve = intercircle2.getInsetSeparateLoopsFromLoops(-outsetRadius, rotatedCarve)
	...
	def addThreadsBridgeLayer(self, layerIndex, nestedRings, rotatedLayer):
		...
		#euclidean2.addToThreadsRemove(extrusionHalfWidth, nestedRings, self.oldOrderedLocation, threadSequence)
		for nestedRing in nestedRings:
	...
	def getGridPoints(self, fillLoops, reverseRotation):
		...
		if self.infillSolidity > 0.8:
			return []
		rotationBaseAngle = euclidean2.getWiddershinsUnitPolar(self.infillBeginRotation)
		...
		for fillLoop in fillLoops:
			rotatedLoops.append(euclidean2.getPointsRoundZAxis(reverseRotationBaseAngle, fillLoop))
		return self.getGridPointsByLoops(gridRotationAngle, intercircle2.getInsetSeparateLoopsFromLoops(self.gridInset, rotatedLoops))

	def getGridPointsByLoops(self, gridRotationAngle, loops):
		'Get the grid points by loops.'
		gridIntersectionsDictionary = {}
		gridPoints = []
		euclidean2.addXIntersectionsFromLoopsForTable(loops, gridIntersectionsDictionary, self.gridRadius)
	...
	def getLayerRotation(self, layerIndex):
		...
		rotation = self.gcode.rotatedLoopLayers[layerIndex].rotation
		if rotation != None:
			return rotation
		...
		return euclidean2.getWiddershinsUnitPolar(layerAngle)
	
	def getLayerRotation2(self, layerIndex, rotatedLayer):
		'Get the layer rotation.'
	...
	def parseLine(self, lineIndex):
		'Parse a gcodeCodec line and add it to the fill skein.'
		...
		if firstWord == 'G1':
			...
			self.nestedRing = euclidean2.NestedBand()
